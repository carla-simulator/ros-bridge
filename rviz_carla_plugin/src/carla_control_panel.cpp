  ...
  : rviz::Panel( parent )
{
  ...
  mEgoVehicleStatusSubscriber = mNodeHandle.subscribe("/carla/ego_vehicle/vehicle_status", 1000, &CarlaControlPanel::egoVehicleStatusChanged, this);
  mEgoVehicleOdometrySubscriber = mNodeHandle.subscribe("/carla/ego_vehicle/odometry", 1000, &CarlaControlPanel::egoVehicleOdometryChanged, this);

  mCameraPosePublisher = mNodeHandle.advertise<geometry_msgs::PoseStamped>("carla/ego_vehicle/spectator_pose", 10, true);

  mEgoVehicleControlManualOverridePublisher = mNodeHandle.advertise<std_msgs::Bool>(
    "/carla/ego_vehicle/vehicle_control_manual_override", 1, true);

  mExecuteScenarioClient = mNodeHandle.serviceClient<carla_ros_scenario_runner_types::ExecuteScenario>("/scenario_runner/execute_scenario");
  mScenarioRunnerStatusSubscriber = mNodeHandle.subscribe("/scenario_runner/status", 10, &CarlaControlPanel::scenarioRunnerStatusChanged, this);

  mTwistPublisher = mNodeHandle.advertise<geometry_msgs::Twist>( "/carla/ego_vehicle/twist", 1 );

  mScenarioSubscriber = mNodeHandle.subscribe("/carla/available_scenarios", 1, &CarlaControlPanel::carlaScenariosChanged, this);
...
void CarlaControlPanel::cameraPreRenderScene(Ogre::Camera* cam)
{
  ...
  if (
    (fabs(cam->getPosition().x - mCameraCurrentPosition.x) > epsilon) ||
    (fabs(cam->getPosition().y - mCameraCurrentPosition.y) > epsilon) ||
    (fabs(cam->getPosition().z - mCameraCurrentPosition.z) > epsilon) ||
    (fabs(cam->getOrientation().x - mCameraCurrentOrientation.x) > epsilon) ||
    (fabs(cam->getOrientation().y - mCameraCurrentOrientation.y) > epsilon) ||
    (fabs(cam->getOrientation().z - mCameraCurrentOrientation.z) > epsilon) ||
    (fabs(cam->getOrientation().w - mCameraCurrentOrientation.w) > epsilon))
  {
